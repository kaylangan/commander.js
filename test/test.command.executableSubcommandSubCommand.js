...
exec('node ' + bin + ' cache help', function (error, stdout, stderr) {
...
exec('node ' + bin + ' cache clear', function (error, stdout, stderr) {
  stdout.should.equal('cache-clear\n');
  stderr.should.equal('');
});

// should print the default command when passed invalid sub-subcommand
exec('node ' + bin + ' cache nope', function (error, stdout, stderr) {
